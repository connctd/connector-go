...
type ConnectorService interface {
	AddInstallation(ctx context.Context, req InstallationRequest) error
	AddInstance(ctx context.Context, req InstantiationRequest) error
	PerformAction(ctx context.Context, req ActionRequest) (*ActionResponse, error)
}
...
func NewConnectorHandler(subrouter *mux.Router, service ConnectorService, host string, publicKey ed25519.PublicKey) *ConnectorHandler {
	...
	c.r.Path("/installations").Methods(http.MethodPost).Handler(NewSignatureValidationHandler(
		ProxiedRequestValidationPreProcessor("https", host, "/api/v1/installations"), publicKey, c.addInstallation))
	c.r.Path("/instances").Methods(http.MethodPost).Handler(NewSignatureValidationHandler(
		ProxiedRequestValidationPreProcessor("https", host, "/api/v1/instances"), publicKey, c.addInstance))
	c.r.Path("/actions").Methods(http.MethodPost).Handler(NewSignatureValidationHandler(
		ProxiedRequestValidationPreProcessor("https", host, "/api/v1/actions"), publicKey, c.performAction))
	return c
}
